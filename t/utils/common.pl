...
sub create_bdr_group {
    ...
    is( $node->safe_psql( $dbname, 'SELECT bdr.bdr_is_active_in_db()' ),
        't', 'BDR is active on node-a after group create' );
}
...
sub startandjoin_node {
    ...
    if ( defined $check_status && $check_status ) {
        check_join_status(
            $join_node,      $upstream_node,
            $join_node_name, $upstream_node_name
        );
    }
...
# BDR group join
sub bdr_join {
    my ($local_node, $join_node) = @_;
    my $node_name  = $local_node->name();

    $local_node->safe_psql(
        $dbname, qq{
...
sub part_node {
    ...
    $upstream_node->psql( $dbname,
        "SELECT bdr.bdr_part_by_node_names(ARRAY['$part_node_name'])" );
    sleep(2);
    if ( defined $check_status && $check_status ) {
        check_part_status( $part_node, $upstream_node, $part_node_name,
            $upstream_node_name, $check_status );
    }
}
...
sub check_part_status {
    ...
    is(
        ...
        'k',
        qq($part_node_name status on upstream node after part)
    );
    is(
        $part_node->safe_psql(
            $dbname,
        ...
        'k',
        qq($part_node_name status on local node after part)
    );

# After a part if the entry is not removed from bdr.bdr_nodes, next node join hangs on wait.
    $upstream_node->safe_psql( $dbname,
        "DELETE FROM bdr.bdr_nodes WHERE node_name = '$part_node_name' and node_status = 'k'"
    );
...
sub check_join_status {
    ...
    is( $join_node->safe_psql( $dbname, 'SELECT bdr.bdr_is_active_in_db()' ),
    ...
    is(
        ...
        'r',
        qq($join_node_name status on local node)
    );
    is(
        $upstream_node->safe_psql(
            $dbname,
        ...
        'r',
        qq($join_node_name status on upstream node)
    );
}
...
qq{SELECT bdr.bdr_replicate_ddl_command(\$DDL\$ CREATE SEQUENCE public.test_seq; \$DDL\$)}
    );
    sleep(5);
    $node->safe_psql(
        $dbname, qq{
...
sub insert_into_table_sequence {
    ...
    for ( my $i = 1 ; $i <= $no_of_inserts ; $i++ ) {
        $node->safe_psql( $dbname,
            " INSERT INTO public.$table_name(node_name) VALUES('$node_name')" );
    }
    sleep(5);
}

# Check table entries with global sequence
sub check_sequence_table {
    ...
    is(
        $node->safe_psql(
            $dbname, "SELECT node_name FROM public.test_table_sequence"
        ),
        $expected,
        $message
    );
...
sub compare_sequence_table_with_upstream {
    ...
    my $upstream_node   = shift;
    my @nodes           = @_;
    my $upstream_record = $upstream_node->safe_psql( $dbname,
        "SELECT * FROM public.test_table_sequence" );
    foreach my $node (@nodes) {
        my $node_record = $node->safe_psql( $dbname,
            "SELECT * FROM public.test_table_sequence" );
...
sub join_notest {
    ...
    system(
        ...
        "-n",                $join_node_name,
        '-d',                $upstream_node->connstr($dbname),
        '--local-dbname',    $dbname,
        '--local-port',      $join_node->port,
        '--postgresql-conf', "$tempdir/postgresql.conf.$join_node_name"
    );
    sleep(10);
}
...
sub join_under_write_load {
    ...
    if ( $type eq 'logical' ) {
        ...
        $join_thread =
          threads->create( \&join_notest, $node_m, $upstream_node );
    }
    $join_thread->join();
    sleep(10);
...
sub join_under_sequence_write_load {
    ...
    # to the cluster
    my $write_thread = threads->create( \&insert_into_table_sequence,
        $upstream_node, "test_table_sequence", 500 );
    my $join_thread;
    if ( $type eq 'logical' ) {
        initandstart_node($node_m);
        $join_thread = threads->create( \&bdr_join, $node_m, $upstream_node );
...
sub concurrent_insert {
    ...
    my $node_1        = shift;
    my $node_2        = shift;
    my $thread1       = threads->create( \&insert_into_table_sequence,
        $node_1, "test_table_sequence", 50 );
    my $thread2 = threads->create( \&insert_into_table_sequence,
        $node_2, "test_table_sequence", 50 );
    $thread1->join();
    $thread2->join();
    my $node1_entry = $node_1->safe_psql( $dbname,
            "SELECT COUNT(*) FROM test_table_sequence WHERE node_name like '"
          . $node_2->name
          . "%'" );
    my $node2_entry = $node_2->safe_psql( $dbname,
            "SELECT COUNT(*) FROM test_table_sequence WHERE node_name like '"
          . $node_1->name
          . "%'" );
    is( $node1_entry, 50, "All records replicated on " . $node_1->name . "" );
    is( $node2_entry, 50, "All records replicated on " . $node_2->name . "" );
    my $duplicates = $node_1->safe_psql( $dbname,
        "SELECT COUNT(*) FROM test_table_sequence GROUP BY id HAVING COUNT(*) > 1"
    );
